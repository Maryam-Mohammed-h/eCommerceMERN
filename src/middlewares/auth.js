  ...
  return async (req, res, next) => {
    try {
      ...
      try {
        ...
        if (
          parseInt(findUser.changePasswordAt?.getTime() / 1000) >
          decodedData.iat
        ) {
          return next(
            new Error("Please login again with new password", { cause: 400 })
          );
        }
        if (!roles.includes(findUser.role)) {
          return next(
            new Error("Unauthorized to access this api ", { cause: 400 })
          );
        }
